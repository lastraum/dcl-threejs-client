...
| Portable experiences (**PX**) | Second full scene layer — **all scene features**, no parcel bounds — law [PORTABLE_EXPERIENCE_COD.md](./PORTABLE_EXPERIENCE_COD.md) · plan [SCENE_LAYERS_PLAN.md](./SCENE_LAYERS_PLAN.md) · claims/HostPoseMode **not landed** (mirror + PX pump debt). **PE** = PointerEvents. |
...
1. Product **1.8** on `yoga-revamp`: scene UI text-measure / Yoga COD finish · **PX layers A→E** ([PORTABLE_EXPERIENCE_COD.md](./PORTABLE_EXPERIENCE_COD.md)) · backpack outfits · **create-community / invites**.  
