...
> **Last updated:** 2026-08-19 (**v2.2.0**). One guest clock (SceneLoop). **v2.1.0** was local preview + stay-in-play reload + Tags shaders (this client; `tjs.sync` opt-in). **v2.0.0** was host world + city walk. **In-scene ECS UI = production / smoke-pass** (one-off bugs only). See [PROGRESS.md](./PROGRESS.md)
