...
| **Date** | 2026-08-18 |
| **Status** | Draft — **do not tag 2.2 yet** |
| **Product version today** | **v2.1.0** (`package.json` / `main`) |
| **Working tree** | Clock PRs #64–#69 on `dev-latest`. Official-scene QA on `fix/snowdrift-ui-sky-avatar-parity` (`7db8931`) — UI / snow / plaza / `/goto` GPU / VFX prime. That QA is **not** the 2.2 headline. |
| **Tip (2026-08-18)** | `main` = v2.1.0. SceneLoop 🟡 until the pointer + Tween + timer walk-log is pasted. |
...
### Release call (2026-08-18)

**Do not tag 2.2 now.** The 2.2 bar in this doc is still: dual-clock landmine sealed **and** a pasted pointer + Tween + timer walk-log. Landmine is sealed (PR-2). Walk-log is not. Official-scene QA on `fix/snowdrift-ui-sky-avatar-parity` (Snow Drift UI/step, plaza mural/hair/chat/props, `/goto` GPU cache, Ice/Cinder overlay prime) is a **2.1.x `dev-latest` QA cut**. Merge that to `dev-latest`, paste the walk-log (`?sceneloop=1` on plaza Cast Line + bobber + `Fishing_Idle`), **then** flip SceneLoop 🟢 and tag 2.2.

Tagging 2.2 today would ship a yellow clock as if it were law.
