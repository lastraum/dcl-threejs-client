...
| **Date** | 2026-08-18 |
| **Status** | Draft |
| **Product version today** | **v2.1.0** (`package.json`) |
| **Working tree** | `feat/hot-reload` tracking `origin/feat/hot-reload`, merged up to latest `origin/dev-latest` |
| **Tip (2026-08-18)** | `4cae971` — v2.1.0 local preview + this-client shaders + unfocused-tab pace + lazy shaders until Jump In |
| **Reference (do not rename this client)** | Bevy/Unity Explorer **platform law** |
| **Our clock name** | SceneLoop / host invert |
| **This plan** | [`docs/V2.2_BEVY_PARITY.md`](docs/V2.2_BEVY_PARITY.md) |

---

## Overview

v2.0 shipped **who owns truth** (host store is the world; official `bin/scene.js` is a guest VM). v2.1 shipped **local preview + stay-in-play reload + this-client Tag shaders** and explicitly deferred the invert clock. [`docs/PROGRESS.md`](docs/PROGRESS.md) keeps SceneLoop 🟡 and points here + [`docs/SCENELOOP_COMPLETION.md`](docs/SCENELOOP_COMPLETION.md). Do **not** flip 🟢 until the walk-log and dual-clock seal land.
...
**P0–P4 = in-tree with named leftovers, not “landed.”** Do not copy-paste “P0–P4 landed” into the law doc. PROGRESS still marks the clock yellow, leftover extra-clock APIs remain (including a live `tickPlayFrame()` outside `SceneLoop.send`), `tickSync` can still start a play-frame if a flag drops, and there is no recorded walk-log that proves pointer edges + guest-`dt` Tweens/timers on this cut.
...
| Dual play-frame (SceneLoop.send **and** `tickSync` → `tickPlayFrame`) | **Landmine**, not the default walk | `SceneWorkerSlot.tickSync` still *can* call `tickPlayFrame` if `!skipPlayFrame && !playFrameOwnedExternally`. Happy path sets the flag at `World.attachMultiScene`. `peTickIntervalMs() === 0` makes PE pump every async frame **if** that flag drops. `secondaryTickIntervalMs()` is already **50** — not the death value |
...
- Host store is the world ([`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)).
...
| WSP systems pie | Parked ([`docs/WORKER_SYSTEM_PIE_V2.md`](docs/WORKER_SYSTEM_PIE_V2.md)); encode/send was the bill |
...
| Full residency rewrite (shells → disc → guests → promote) | [`docs/OPEN_WORLD_RESIDENCY.md`](docs/OPEN_WORLD_RESIDENCY.md) is its own chapter. 2.2 only owns the **clock law** applied to live guests that are already on |
...
| --- | --- | --- | --- | --- |
| Host store is the world | Native host owns components / PhysX / present | [`ARCHITECTURE.md`](docs/ARCHITECTURE.md); `CrdtProjection` + `EntityStore` + `PhysXWorld` | **law** | no |
...
| Raycast cadence | Once per guest tick | [`RaycastSystem.sync`](src/input/RaycastSystem.ts) only from `SceneScriptSystem.updateRaycasts` ← `tickPlayFrame`. Comment still says "every frame" | **law** (comment stale) | docs |
...
| Dual-clock death | `tickSync` never `tickPlayFrame` | Owned-externally skips play-frame. `SceneWorkerSlot.tickSync` still *can* call `tickPlayFrame` if the flag is false. `peTickIntervalMs() === 0` | **partial** | **must-seal before 🟢** (PR-2) |
...
| PE P3 pad/wind | Explorer-raw feel | Implementation 🟢; manual QA open ([`PHYSICS_PARITY_PLAN.md`](docs/PHYSICS_PARITY_PLAN.md)) | **partial** | should if clock early |
| Platform goldens | Plane UV / AvatarAttach quat / VC lookAt | Open gaps ([`PLATFORM_COMPONENT_LAWS.md`](docs/PLATFORM_COMPONENT_LAWS.md) §10) | **partial** | measure-only |
...
That note is now corrected: 🟡 remains, with pointers at this plan + [`docs/SCENELOOP_COMPLETION.md`](docs/SCENELOOP_COMPLETION.md). [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) related table points at both.

[`docs/SCENELOOP_COMPLETION.md`](docs/SCENELOOP_COMPLETION.md) is the committed target-shape law. An earlier draft said **“P0–P4 landed.”** That overclaim is gone. Verify leftovers against this tree.
...
[`RaycastSystem.sync`](src/input/RaycastSystem.ts) is only called from `SceneScriptSystem.updateRaycasts()` ← `tickPlayFrame()`. Continuous identical payloads skip LWW PUT (`lastContinuousSig`). The class comment “Run each frame before encoder flush” is stale.
...
[`docs/OPEN_WORLD_RESIDENCY.md`](docs/OPEN_WORLD_RESIDENCY.md) (2026-08-13) described kill-switch city fill and `secondaryTickIntervalMs() === 0`. **That snapshot is stale vs this tree.** Live guests are on. Dual-clock death is a **shipping landmine on the ownership boolean**, not a currently-on `interval === 0` path.
...
- Commit this plan as [`docs/V2.2_BEVY_PARITY.md`](docs/V2.2_BEVY_PARITY.md).
- Commit [`docs/SCENELOOP_COMPLETION.md`](docs/SCENELOOP_COMPLETION.md) after rewriting the stale “still not the law” table and marking P0/P1 **in-tree with named leftovers** (not “landed”).
- Point PROGRESS at both docs. Keep SceneLoop 🟡 until the walk-log **and** the dual-clock landmine are sealed.
- Stale comments: `RaycastSystem.ts` L65 (“every frame”); `SceneWorkerSlot.ts` L539 (“Secondary scripts run on tickSync every frame”); `SecondaryLiveManager.ts` L1044 (“Scripts still run every sync frame (tickSync)”).
...
```text
[sceneloop] play-frame source=play-frame dt=0.050 inFlight=0 guests=1 sent=1 raycast=1 trigger=0
```
...
PROGRESS SceneLoop → 🟢 only if (1) this walk-log is pasted **and** (2) PR-2 dual-clock landmine has landed. Do not flip a single SceneLoop 🟢 while `tickSync` can still start `tickPlayFrame`.
...
Must-do: node/unit test that `tickSync` / `tickStickySync` never call `tickPlayFrame` (guards PR-2).

Measure-only (decide from a stacked-neighbor walk log; do not change compile defaults in the same PR unless p5 < 30 is proven). *Guide:* Genesis CBD.
...
| last guest `dt` | `MainFrameHud.ts` SceneLoop line (missing today) | same as play-frame fail window |
...
| PR-3 walk-log + last-`dt` HUD | `?sceneloop=1` optional verbose | paste log in PR | hide flag |
...
- [`docs/V2.2_BEVY_PARITY.md`](docs/V2.2_BEVY_PARITY.md) — this plan
- [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) — host invert law
- [`docs/SCENELOOP_COMPLETION.md`](docs/SCENELOOP_COMPLETION.md) — target shape + leftover inventory (P0/P1 not “landed”)
- [`docs/PROGRESS.md`](docs/PROGRESS.md) — v2.1.0 deferred the clock
- [`docs/AGENTS.md`](docs/AGENTS.md) — scene-bundle-is-law · FocusOwner
- [`docs/INTEGRATION.md`](docs/INTEGRATION.md) — ECS complete; shell yellows
- [`docs/PLATFORM_COMPONENT_LAWS.md`](docs/PLATFORM_COMPONENT_LAWS.md) — TweenSequence · open goldens
- [`docs/OPEN_WORLD_RESIDENCY.md`](docs/OPEN_WORLD_RESIDENCY.md) — dual-clock death (snapshot partly stale)
- [`docs/MULTI_SCENE_CONTINUITY.md`](docs/MULTI_SCENE_CONTINUITY.md) — FocusOwner / live vs shells
- [`docs/PHYSICS_PARITY_PLAN.md`](docs/PHYSICS_PARITY_PLAN.md) — PE P3 pad/wind QA
- [`docs/CLAIMS.yaml`](docs/CLAIMS.yaml) — PR 62 / 56 pending_review
...
- **Description:** Instrument, run an official bundle that uses pointer + Tween + scene timers, paste the log. *Guide:* Genesis Plaza fishing. Pass = aim tracks, PET_UP on the authored hit, Tween duration ≈ wall, scene timer fires on real `dt`, no `dt=0.000` after first `source=play-frame`. Same checks apply to any other official bundle that uses those APIs. This is the 2.2 must-ship **proof** gate; the landmine seal is already in PR-2.

### PR-4 — Neighbor FPS measure (not the 🟢 gate)

- **Title:** `clock: stacked live-guest FPS soak (measure-only)`
- **Files/components:** `src/dcl/multiScene/caps.ts` (boot concurrency **only if** the walk log proves p5 < 30); `src/core/World.ts` — **only** `aoiVisual.update` (far shells / distance fade) may move off present if the stacked-neighbor log proves a present hitch; **do not** move `scenePromote.tick` (URL / location pill / minimap) or `syncCurrentSceneGuestAt` (whose guest your feet are in). Keep the PR-2 `skipPlayFrame` test green
- **Dependencies:** PR-2. May parallelize with PR-3. **Does not block SceneLoop 🟢.**
- **Description:** Must-do is the regression test that `tickSync` never `tickPlayFrame` (if not already in PR-2). Measure-only: boot concurrency (tree = 4); whether AOI visuals hitch the render frame. Soft-route and Focus stay on present. Soak: any dense live-neighbor walk (*guide:* Genesis CBD), `?aoilive=0` still primary-only. Do not change compile defaults in this PR unless the pasted log fails p5 ≥ 30. Same bar on Worlds / any multi-scene island.
