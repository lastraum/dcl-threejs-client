...
import type { RealmEndpoints, ResolvedScene, SceneMetadata } from './types'
...
import { resolvePreviewScene } from './resolvePreviewScene'
import { resolvedFromEntity } from './resolveSceneEntity'

export { assertSdk7CompatibleScene } from './resolveSceneEntity'
...
export async function resolveSceneFromRoute(target: RouteTarget): Promise<ResolvedScene> {
  ...
  if (target.kind === 'preview') {
    return resolvePreviewScene(target.realmUrl)
  }
...
export function summarizeSceneContent(scene: ResolvedScene): string {
  ...
  if (scene.source.kind === 'preview') {
    lines.push(`Preview: ${scene.source.realmUrl}`)
  }
