...
export async function fetchWorldDeployDisplayMeta(
  ...
  let imageUrl = thumb ? resolveSceneAssetRef(thumb, content, assetUrl) : null
  if (!imageUrl) {
    for (const file of [
      'scene-thumbnail.png',
      ...
      imageUrl = resolveSceneAssetRef(file, content, assetUrl)
      if (imageUrl) break
    }
  }
  if (!imageUrl) {
    const nested = content.find((row) => row.file.endsWith('/scene-thumbnail.png'))
    if (nested) imageUrl = assetUrl(nested.hash)
  }
...
export async function resolveSceneFromRoute(target: RouteTarget): Promise<ResolvedScene> {
  ...
  if (target.kind === 'forest') {
    throw new Error('Worlds route does not resolve a network scene')
  }
