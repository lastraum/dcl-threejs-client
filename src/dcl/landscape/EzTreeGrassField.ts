...
/**
 * Explorer empty-parcel grass: occupancy-only (caller passes vacant keys).
 * Unity `GrassIndirectRenderer` plants 256 blades/parcel on a 16×16 grid via
 * compute; WebGL uses an 8×8 sample (64) of that same grid + ez-tree wind.
 * No ez-tree patchiness — Explorer occupancy is the only cull.
 */
export const VACANT_GRASS_STEP = 2

function collectVacantParcelGrassInstances(
  parcelKeys: string[],
  primaryBase: string,
  bladeTint: THREE.Color
): GrassInstance[] {
  ...
  for (const key of parcelKeys) {
    ...
    for (let gy = 0; gy < PARCEL_SIZE; gy += VACANT_GRASS_STEP) {
      for (let gx = 0; gx < PARCEL_SIZE; gx += VACANT_GRASS_STEP) {
        const cellRng = mulberry32(hashParcelCoords(parcel.x * 16 + gx, parcel.y * 16 + gy, 5))
        const dclX = originX + gx + cellRng() * VACANT_GRASS_STEP
        const dclZ = originZ + gy + cellRng() * VACANT_GRASS_STEP
        const three = dclSceneToLandscapeThree(dclX, dclZ, base)
        const scale = grassInstanceScale(cellRng)
        instances.push({
          x: three.x,
          y: EMPTY_LAND_GROUND_OFFSET.y,
          z: three.z,
          ...
          scaleY: scale.y,
          scaleZ: scale.z,
          color: grassInstanceColor(cellRng, bladeTint)
        })
      }
    }
  }
...
export async function buildVacantParcelGrassField(
  parcelKeys: string[],
  primaryBase: string,
  options?: { windShader?: boolean; grassColor?: THREE.Color | string | number }
): Promise<EzTreeGrassFieldHandle | null> {
  ...
  const instances = collectVacantParcelGrassInstances(parcelKeys, primaryBase, bladeTint)
  if (!instances.length) return null

  const windShader = options?.windShader !== false
  const sourceMesh = await fetchGrassMesh()
  const map = extractMaterialMap(sourceMesh.material)
  if (!map) throw new Error('vacant grass: texture unavailable after load')

  const grassMaterial = createGrassMaterial(map, bladeTint, windShader)
  // Clone so AOI scatter purge can dispose without touching the shared template.
  const grassMesh = new THREE.InstancedMesh(
    sourceMesh.geometry.clone(),
    grassMaterial,
    instances.length
  )
  grassMesh.name = 'aoi-vacant-grass'
  grassMesh.receiveShadow = true
  grassMesh.castShadow = false
  grassMesh.count = instances.length
  ...
  const group = new THREE.Group()
  group.name = 'aoi-vacant-grass-field'
  group.add(grassMesh)
  ...
  const update = (elapsed: number, cameraPos: THREE.Vector3): void => {
    if (windShader) setGrassWindElapsed(grassMaterial, elapsed)
    const now = performance.now()
    if (now - lastLodUpdate < 160) return
    lastLodUpdate = now
    ...
    let target = 0
    for (let i = 0; i < sortedByDist.length; i++) {
      if (sortedByDist[i]!.distSq > farSq) break
      target++
    }
    target = Math.max(1, Math.floor(target * 0.85 + instances.length * LOD_MIN_FRACTION * 0.15))
    target = Math.min(target, instances.length)
