...
export type LocalProjectMeta = {
  ...
  /**
   * Relative path from scene folder root to thumbnail image
   * (usually display.navmapThumbnail from scene.json).
   */
...
/** Creator Hub / scene.json thumbnail candidates (relative to project root). */
function resolveSceneThumbnailPath(json: {
  display?: { title?: string; navmapThumbnail?: string }
}): string | undefined {
  const nav = json.display?.navmapThumbnail?.trim()
  if (nav) return nav.replace(/^\.\//, '')
  // Common fallbacks when navmapThumbnail is missing
  return undefined
}

async function fileExistsInRoot(
  root: FileSystemDirectoryHandle,
  relativePath: string
): Promise<boolean> {
  try {
    await getFileFromRelativePath(root, relativePath)
    return true
  } catch {
    return false
  }
...
/** Walk nested path relative to project root (e.g. assets/scene/Images/PlazaPic.png). */
...
const THUMBNAIL_FALLBACKS = [
  ...
  'assets/scene/Images/scene-thumbnail.png'
] as const

async function resolveThumbnailRelativePath(
  handle: FileSystemDirectoryHandle,
  fromJson: string | undefined
): Promise<string | undefined> {
  if (fromJson && (await fileExistsInRoot(handle, fromJson))) return fromJson
  for (const candidate of THUMBNAIL_FALLBACKS) {
    if (await fileExistsInRoot(handle, candidate)) return candidate
  }
  // Prefer declared navmap even if file missing (UI shows placeholder; re-link may fix)
  return fromJson
}
...
async function readSceneJsonMeta(handle: FileSystemDirectoryHandle): Promise<{
  ...
  try {
    ...
    const fromDisplay = resolveSceneThumbnailPath(json)
    const thumbnailPath = await resolveThumbnailRelativePath(handle, fromDisplay)
    return {
...
/**
 * Load scene thumbnail as a blob: URL for hub cards.
 * Caller should revokeObjectURL when the card is torn down / refreshed.
 */
export async function loadProjectThumbnailObjectUrl(
  projectId: string
): Promise<string | null> {
  ...
  let path = meta?.thumbnailPath

  // Refresh path from scene.json if missing (older localStorage entries).
  if (!path) {
    const sceneMeta = await readSceneJsonMeta(handle)
    path = sceneMeta.thumbnailPath
    if (path && meta) {
      upsertMeta({ ...meta, thumbnailPath: path })
    }
  }
  if (!path) return null

  try {
    const file = await getFileFromRelativePath(handle, path)
    // Creator Hub sometimes omits MIME; allow common image extensions.
    if (!file.type.startsWith('image/') && !/\.(png|jpe?g|webp|gif)$/i.test(path)) {
      return null
    }
...
export async function syncCreatorHubScenes(scenesRoot: FileSystemDirectoryHandle): Promise<CreatorHubSyncResult> {
  ...
  for await (const [name, entry] of scenesRoot.entries()) {
    ...
    const meta: LocalProjectMeta = {
      ...
      source: 'creator-hub',
      folderName: name,
      thumbnailPath: metaFromScene.thumbnailPath
    }
...
export async function registerProjectFromHandle(
  ...
  const meta: LocalProjectMeta = {
    ...
    accessMode: 'fsa',
    thumbnailPath: metaFromScene.thumbnailPath ?? prev?.thumbnailPath
  }
