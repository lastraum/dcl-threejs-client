...
export type Rfc4RouterHandlers = {
  ...
  onPeerPet?: (address: string, data: Uint8Array, transport: TransportType) => void
}
...
export class Rfc4Router {
  ...
  handlePacket(transport: TransportType, address: string, data: Uint8Array): void {
    ...
    if (scenePacket) {
      ...
      if (scenePacket.sceneId === DPET_SCENE_ID) {
        this.handlers.onPeerPet?.(address, scenePacket.data, transport)
        return
      }
