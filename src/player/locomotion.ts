...
export function freezesAvatarFromModifier(mod: unknown): boolean {
  ...
  const mode = (mod as { mode?: { $case?: string; standard?: {
    disableAll?: boolean
    disableWalk?: boolean
    disableJog?: boolean
    disableRun?: boolean
...
/** Build locomotion config from a raw InputModifier component value (PE claim override). */
export function locomotionConfigFromInputModifier(mod: unknown): LocomotionConfig {
  ...
  const mode = (mod as { mode?: { $case?: string; standard?: Parameters<typeof applyInputModifier>[1] } }).mode
  const std = mode?.$case === 'standard' ? mode.standard : undefined
