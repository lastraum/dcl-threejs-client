...
/** Protobuf may emit SDK default 1/3 slices — not a scene-authored nine-slice intent. */
function hasMeaningfulTextureSlices(textureSlices?: PBUiBackground['textureSlices']): boolean {
  if (!textureSlices) return false
  const eps = 0.001
  const top = textureSlices.top ?? DEFAULT_SLICES.top
  const left = textureSlices.left ?? DEFAULT_SLICES.left
  const right = textureSlices.right ?? DEFAULT_SLICES.right
  const bottom = textureSlices.bottom ?? DEFAULT_SLICES.bottom
  const isDefault =
    Math.abs(top - DEFAULT_SLICES.top) < eps &&
    Math.abs(left - DEFAULT_SLICES.left) < eps &&
    Math.abs(right - DEFAULT_SLICES.right) < eps &&
    Math.abs(bottom - DEFAULT_SLICES.bottom) < eps
  return !isDefault
}

/**
 * react-ecs defaults missing textureMode to CENTER (1).
 * Protobuf defaults to NINE_SLICES (0) — treat as stretch/center for scene button art.
 */
export function normalizeBackgroundTextureMode(
  ...
  if (numeric === BackgroundTextureMode.NINE_SLICES && src) {
    if (/^https?:\/\//i.test(src) || !hasMeaningfulTextureSlices(textureSlices)) {
