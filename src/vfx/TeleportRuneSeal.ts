/**
 * Cyan runic seal — GroundField(RUNE) + Tube STRAIGHT + Shell PRESSURE
 * from threejs-vfx (vfx.lastslice.co), recolored off the amber/orange palette.
 *
 * Shared by the /worlds forest Jump In and in-world /goto + peer-leave rituals.
 */
...
/** Time the ritual stays on-screen before Jump In /goto may show the loading overlay. */
export const TELEPORT_RUNE_WATCH_MS = Math.ceil((DONE_AT + HOLD + FADE) * 1000)
...
export type TeleportRuneSealOpts = {
  x: number
  y?: number
  z: number
  yaw: number
...
export class TeleportRuneSeal {
  ...
  constructor(scene: THREE.Scene, name = 'teleport-rune-seal') {
  ...
  /**
   * Compile the RUNE program (and the column) against the live lights / fog
   * so the first present does not hitch.
   */
  async prewarm(
    ...
    try {
      ...
      /* first play still compiles if this program is skipped */
    }
    this.field.setVisible(false)
    this.column.visible = false
    this.wave.visible = false
  ...
  play(opts: TeleportRuneSealOpts): Promise<void> {
    this.age = 0
    this.done = false
    ...
    this.yaw = opts.yaw
    this.centre.set(opts.x, opts.y ?? 0, opts.z)
    this.onBurst = opts.onBurst ?? null
    this.onDischarge = opts.onDischarge ?? null
    this.light.position.set(opts.x, this.centre.y + 1.4, opts.z)
    this.column.visible = false
    this.wave.visible = false
    if (!frame.uSceneDepth.value) frame.uSceneDepth.value = farDepthTexture()
    ...
    return new Promise((resolve) => {
      const fail = window.setTimeout(() => this.finish(), TELEPORT_RUNE_WATCH_MS + 2500)
      const wait = () => {
        if (this.done) {
  ...
  update(dt: number, time: number, resolution: THREE.Vector2, camera: THREE.Camera): void {
    ...
    const shrink =
      this.age < DONE_AT + HOLD ? 0 : saturate((this.age - DONE_AT - HOLD) / FADE)
    const recede = easeInCubic(shrink)
    // Ground stays solid while it collapses; only the last sliver fades.
    const fieldFade = 1 - saturate((shrink - 0.78) / 0.22)
    const columnFade = 1 - easeOutCubic(shrink)
    ...
    this.writeField(inscribe, ignite, discharge, fieldFade, recede)
    this.syncDischarge(discharge, columnFade)

    const lit = ignite * 2.8 + discharge * 4.5
    this.light.intensity = lit * columnFade
    this.light.distance = 10 + this.radius * 2 * (1 - recede * 0.7)
    this.light.position.set(
      this.centre.x,
      this.centre.y + 1.4 + this.fx.columnHeight * 0.35 * discharge * columnFade,
      this.centre.z
    )

    if (shrink >= 1) this.finish()
  }
  ...
  private writeField(
    ...
    p.yaw = this.yaw
    // GroundField places the quad at world Y = height (ignores centre.y).
    p.height = this.centre.y + 0.06
    const visRadius = Math.max(0.12, this.radius * (1 - recede * 0.92))
    p.radius = visRadius
    ...
    p.rings = 3
    p.ringInner = 0.34
    p.glyphSize = Math.min(0.5, Math.max(0.32, visRadius * 0.11))
    p.glyphStroke = 0.036
    p.glyphGap = 1.3
    p.spin = 0.14
    p.spinFalloff = 0.55
    ...
    p.tickMajorLen = 0.16
    p.sigilStart = 0.72
    p.sigilRadius = Math.min(0.42, visRadius * 0.1)
    p.sigilSize = Math.min(0.82, visRadius * 0.18)
    p.sigilArms = 5
    p.sigilStroke = 0.034
    p.sigilRing = 0.98
    p.sigilSpin = 0.07
  ...
  private syncDischarge(discharge: number, fade: number): void {
    ...
    t.origin.y = this.centre.y + 0.06
    t.target.copy(this.centre)
    t.target.y = this.centre.y + 0.06 + Math.max(0.05, c.columnHeight) * rise
    t.side.set(1, 0, 0)
    t.progress = 1
